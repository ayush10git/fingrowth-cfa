...
const SingleMockResultVisualization = ({ mockId }) => {
  const mockData = [
    {
      id: 1,
      name: "Mock 1",
  ...
  const selectedMock = mockData.find((mock) => mock.id === Number(mockId));

  if (!selectedMock) {
    return <div className="text-center text-red-500">Mock test not found</div>;
  }

  const totalScore = selectedMock.subjects.reduce(
  ...
  return (
    <div className="p-4 bg-white rounded-md flex flex-col gap-4">
      <h1 className="text-xl font-bold mb-8">{selectedMock.name} Result</h1>

      <div className="mb-6">
        <div className="flex items-center mb-2">
          <span className="font-medium w-16">{selectedMock.name}</span>
          <div className="flex-1 h-8 rounded-lg overflow-hidden flex">
            {selectedMock.subjects.map((subject) => {
              const widthPercentage = (subject.score / totalScore) * 100;

              return (
                <Tooltip
                  key={`${selectedMock.id}-${subject.name}`}
...
export default SingleMockResultVisualization;
